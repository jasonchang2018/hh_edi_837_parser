...
(
    ...
    where       case    when    debtor.pl_group = 'IU HEALTH - TPL'
                ...
                and cdn_ not in (select cdn from edwprodhh.edi_837p_parser.export_data_dimensions_accounts_log where upload_date >= current_date() - 7)     --if selected but not sent to vendor, try again after cooldown period
    limit       100
)
select      debtor_idx,
            drl_            as drl,

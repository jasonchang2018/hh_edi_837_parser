create or replace table
    edwprodhh.edi_837p_parser.header_interchange_control
as
with labeled as
(
...
create or replace task
    edwprodhh.edi_837p_parser.insert_header_interchange_control
...
(
    ...
    where       regexp_like(flatten_837.line_element_837, '^ISA.*')                         --1 Filter
                and flatten_837.response_id not in (select response_id from edwprodhh.edi_837p_parser.header_interchange_control)
)
select      response_id,
            interchange_control_header,
            authorization_information_qualifier,

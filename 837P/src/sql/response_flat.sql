create or replace table
    edwprodhh.edi_837p_parser.response_flat
as
with flattened as
(
    select      response_id,
...
(
    select      *,
                max(case when regexp_like(line_element_837, '^SVD.*')               then index end) over (partition by response_id, nth_transaction_set, claim_index, lx_index order by index asc) as svd_index
    from        add_lx_index
)
select      response_id,
            file_date,
...
create or replace task
    edwprodhh.edi_837p_parser.insert_response_flat
    warehouse = analysis_wh
    after edwprodhh.edi_837p_parser.sp_insert_837p_from_stage
as
insert into
    edwprodhh.edi_837p_parser.response_flat
(
...
(
    select      response_id,
                ...
                trim(regexp_replace(regexp_replace(response_body, '\\s+', ' '), '~', '')) as line_element_837
    from        edwprodhh.edi_837p_parser.response
    where       response_id not in (select response_id from edwprodhh.edi_837p_parser.response_flat)
)
, add_transaction_set_index as
(  
    select      *,

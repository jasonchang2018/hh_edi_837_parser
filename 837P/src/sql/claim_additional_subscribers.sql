create or replace table
    edwprodhh.edi_837p_parser.claim_additional_subscribers
as
...
(
    ...
    (
        select      filtered_clm_sbr.response_id,
                    ...
                    case    when    flattened.index = 1   then      'PREFIX_MOA'
                            when    flattened.index = 2   then      'MOA_REIMBURSE_CODE'
                            when    flattened.index = 3   then      'MOA_HCPCS_PAYABLE_AMT'
                            when    flattened.index = 4   then      'MOA_REMARK_1'
                            when    flattened.index = 5   then      'MOA_REMARK_2'
                            when    flattened.index = 6   then      'MOA_REMARK_3'
                            when    flattened.index = 7   then      'MOA_REMARK_4'
                            end     as value_header,
        ...
        where       regexp_like(filtered_clm_sbr.line_element_837, '^MOA.*')                         --1 Filter
    )
    select      *
    from        long
                pivot(
...
(
    ...
    (
        select      filtered_clm_sbr.response_id,
                    ...
                    case    when    flattened.index = 1   then      'DTP_PREFIX_SBR_ADJUDICATION'
                            when    flattened.index = 2   then      'DATE_QUALIFIER_SBR_ADJUDICATION'
                            when    flattened.index = 3   then      'DATE_FORMAT_SBR_ADJUDICATION'
                            when    flattened.index = 4   then      'DATE_SBR_ADJUDICATION'
                            end     as value_header,

                    case    when    value_header = 'DATE_SBR_ADJUDICATION'
        ...
        where       regexp_like(filtered_clm_sbr.line_element_837, '^DTP\\*573.*.*')                         --1 Filter
    )
    select      *
    from        long
                pivot(
                    max(value_format) for value_header in (
                        'DTP_PREFIX_SBR_ADJUDICATION',
                        'DATE_QUALIFIER_SBR_ADJUDICATION',
                        'DATE_FORMAT_SBR_ADJUDICATION',
                        'DATE_SBR_ADJUDICATION'
                    )
                )   as pvt (
                    RESPONSE_ID,
                    NTH_TRANSACTION_SET,
    ...
    (
        select      filtered_clm_sbr.response_id,
                    ...
                    case    when    flattened.index = 1   then      'REF_PREFIX_CLAIM'
                            when    flattened.index = 2   then      'REFERENCE_ID_CODE_CLAIM'
        ...
        where       regexp_like(filtered_clm_sbr.line_element_837, '^REF.*')                         --1 Filter
    )
    , pivoted as
    (
        select      *
...
select      header.response_id,
            ...
            moa.prefix_moa,
            moa.moa_reimburse_code,
            moa.moa_hcpcs_payable_amt,
            moa.moa_remark_1,
            moa.moa_remark_2,
            moa.moa_remark_3,
            moa.moa_remark_4,
            dtp_573.dtp_prefix_sbr_adjudication,
            dtp_573.date_qualifier_sbr_adjudication,
            dtp_573.date_format_sbr_adjudication,
            dtp_573.date_sbr_adjudication,
...
from        claim_sbr_header    as header
            ...
            left join
                claim_sbr_moa       as moa
                on  header.response_id          = moa.response_id
                and header.nth_transaction_set  = moa.nth_transaction_set
                and header.claim_index          = moa.claim_index
                and header.other_sbr_index      = moa.other_sbr_index
            left join
                claim_sbr_dtp_573   as dtp_573
                ...
                and header.other_sbr_index      = dtp_573.other_sbr_index
            left join
            ...
            left join
                ...
                and header.other_sbr_index      = ref.other_sbr_index
...
create or replace task
    edwprodhh.edi_837p_parser.insert_claim_additional_subscribers
...
(
    select      *
    from        edwprodhh.edi_837p_parser.response_flat
    where       response_id not in (select response_id from edwprodhh.edi_837p_parser.claim_additional_subscribers)

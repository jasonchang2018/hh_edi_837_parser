...
(
    ...
    (
        ...
        where       regexp_like(flatten_835.line_element_835, '^ISA.*')                         --1 Filter
    )
    select      *,
...
(
    ...
    (
        ...
        where       regexp_like(flatten_835.line_element_835, '^IEA.*')                         --1 Filter
    )
    select      *
    from        long
                pivot(
...
create or replace task
    edwprodhh.edi_835_parser.insert_header_interchange_control
...
-- and flatten_835.response_id not in (select response_id from edwprodhh.edi_835_parser.header_interchange_control)
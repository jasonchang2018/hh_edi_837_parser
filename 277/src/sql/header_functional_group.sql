create or replace table
    edwprodhh.edi_277_parser.header_functional_group
as
with header_gs as
(
    with long as
    (
        select      flatten_277.response_id,
                    flatten_277.nth_functional_group,
        ...
        from        edwprodhh.edi_277_parser.response_flat as flatten_277,
                    lateral split_to_table(flatten_277.line_element_277, '*') as flattened      --2 Flatten

        where       regexp_like(flatten_277.line_element_277, '^GS.*')                          --1 Filter
    )
    select      *,
...
(
    ...
    (
        ...
        where       regexp_like(flatten_277.line_element_277, '^GE.*')                          --1 Filter
    )
    select      *
    from        long
                pivot(
...
create or replace task
    edwprodhh.edi_277_parser.insert_header_functional_group
...
(
    ...
    (
        ...
        where       regexp_like(flatten_277.line_element_277, '^GS.*')                          --1 Filter
                    and flatten_277.response_id not in (select response_id from edwprodhh.edi_277_parser.header_functional_group)
    )
    select      *,
                (functional_group_created_date || ' ' || functional_group_created_time)::timestamp as functional_group_created_timestamp
    from        long

create or replace table
    edwprodhh.edi_277_parser.header_interchange_control
as
with header_isa as
(
    with long as
    (
        ...
        where       regexp_like(flatten_277.line_element_277, '^ISA.*')                         --1 Filter
    )
    select      *,
...
(
    ...
    (
        ...
        where       regexp_like(flatten_277.line_element_277, '^IEA.*')                         --1 Filter
    )
    select      *
    from        long
                pivot(
...
create or replace task
    edwprodhh.edi_277_parser.insert_header_interchange_control
...
(
    ...
    (
        ...
        where       regexp_like(flatten_277.line_element_277, '^ISA.*')                         --1 Filter
                    and flatten_277.response_id not in (select response_id from edwprodhh.edi_277_parser.header_interchange_control)
    )
    select      *,
                (interchange_date || ' ' || interchange_time)::timestamp as interchange_timestamp
    from        long
